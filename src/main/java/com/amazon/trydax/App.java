...
public class App {
    public static void main(String[] args) throws Exception {
        ...
        if (args.length >= 1) {
            System.out.println("******USING DAX CLIENT!!******");
    ...
    private static void getItemTest(String tableName, DynamoDbAsyncClient client, int pk, int sk, int iterations) {
        long startTime, endTime, successCount = 0;
                    ...
                    for (int isk = 1; isk <= sk; isk++) {
                        try {
                            client.getItem(GetItemRequest.builder()
                                    ...
                                    .get();
                                    successCount++;
                        } catch (java.util.concurrent.ExecutionException exec) {
                            System.out.println("Failed to get item " + exec.getMessage());
                        }
                    }
                }
            } catch (Exception e) {
